...
//ABIs
// import pair_ABI from '../contract_ABI/pairABI.js'
// import router_ABI from '../contract_ABI/routerABI.js'
const pair_ABI = [
...
let tokenIn = document.getElementById('tokenIn');
let tokenOut = document.getElementById('tokenOut');

let amountIn = document.getElementById('amountIn');
const buttonSwap = document.getElementById('swapT')
const buttonChange = document.getElementById('changeUnit')
...
async function swapToken() {
    ...
    } else if (document.getElementById('tokenIn').textContent == "CAB") {
...
async function updateValue() {
    const swapContract = new ethers.Contract("0x76cCA629cF6AEC3204c35584F85bA24a1De54149", pair_ABI, provider.getSigner())
    const getReserves = await swapContract.getReserves()
    let rs1 = getReserves[0]
    let rs2 = getReserves[1]

    if (document.getElementById('tokenIn').textContent == "USDT") {
        let rate = rs1 / rs2
        const epValue = amountIn.value * rate
        amountOut.value = epValue
    } else if (document.getElementById('tokenIn').textContent == "CAB") {
        let rate = rs2 / rs1
        const epValue = amountIn.value * rate
        amountOut.value = epValue
...
function swapImages() {
    let get1 = document.getElementById("imgIn");
    let get2 = document.getElementById("imgOut");
    let fetch = get1.src;
    get1.src = get2.src;
    get2.src = fetch;
}
...
// //contracts
// const router = new ethers.Contract("0xb3fCf70006119745Cb241ee78AD43a988EE6Fd41", router_ABI, signer);

// //asyncs variables

// const pair = new ethers.Contract("0x76cCA629cF6AEC3204c35584F85bA24a1De54149", pair_ABI, signer);

function defaultNotificationTimer() {
    new NotifyJS(
        {
            duration: 5000,
            message: "Swap successfully!<br>This notification will auto-close in",
            timer: true
        }
    )
}
